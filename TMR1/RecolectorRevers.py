...
# Umbrales y tiempos
LINE_REFLECT_THRESH = 10
LINE_REFLECT_THRESH_DER = 8  # umbral menos sensible para el sensor derecho (INPUT_4)
DIST_THRESH_CM = 3.0
STOP_DIST_CM = 1.0
TIEMPO_ESPERA_SENSOR = 2.0  # segundos para esperar deteccion de objeto
RGB_LIMITE = 15              # umbral de reflexion para detectar objeto en garra (reducido -> mayor sensibilidad)
...
class LineFollowerRobot:
    ...
    def seguir_linea_step(self):
        ...
        # Error simple: diferencia entre sensores
        error = iz - de
        # Al conducir en reversa, invertir la contribucion de giro
        Kp = 0.8
        base = VEL_NORMAL
        turn = Kp * error
        ...
        # Para marcha atrás, mover motores con velocidades negativas (base ya negativo)
    ...
    def buscar_pelotas_en_medio(self):
        ...
        while not self.btn.any():
            ...
            # Si ambos sensores detectan negro, terminamos la busqueda y detenemos
            if iz_line and de_line:
                print("Cruce de linea detectado (sensores 3 y 4). Finalizando busqueda.")
                self.tank.off()
                return True
