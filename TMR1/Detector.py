...
ULTRA_SLOW_DISTANCE_CM = 15  # distancia en cm para empezar a reducir velocidad
BLACK_THRESHOLD = 20         # umbral de luz reflejada para detectar línea negra
CONSECUTIVE = 1              # lecturas consecutivas requeridas (puede aumentarse si hay ruido)

# --- Función principal que implementa la secuencia solicitada ---
def main_sequence():
    try:
        # 1) Ejecutar la rotación inicial exacta solicitada
        print("Paso 1: on_for_rotations(-30, -30, 2.87)")
        tank_drive.on_for_rotations(-30, -30, 2.87)
        tank_drive.off()
        time.sleep(0.2)

        # 2) Avanzar en línea recta y reducir velocidad al detectar objeto a 10 cm
        print("Paso 2: avanzar en línea recta; reducir velocidad si objeto a <= {} cm".format(ULTRA_SLOW_DISTANCE_CM))
        left_stopped = False
        right_stopped = False
        left_count = 0
        right_count = 0
        ...
        # Bucle principal: avanza hasta que ambos sensores de línea hayan detectado la línea negra
        while True:
            # Lectura ultrasonido
            try:
            ...
            if dist is not None and dist <= ULTRA_SLOW_DISTANCE_CM:
                ...
                if current_left_speed != SLOW_SPEED or current_right_speed != SLOW_SPEED:
                    ...
                    # Si alguno de los motores ya está detenido por la línea, mantenerlo detenido
            ...
            # Si no hay objeto cercano, mantener velocidad normal (si no se ha detenido por línea)
            else:
            ...
            # Detección de línea negra por cada sensor
            ...
            # Si el sensor izquierdo detecta la línea (primer sensor que la toque)
            if left_count >= CONSECUTIVE and not left_stopped:
                ...
                print("Sensor izquierdo detectó línea -> detener motor izquierdo y esperar al derecho.")
                # left motor = 0, right motor = current_right_speed (si no detenido)
                tank_drive.on(0, 0 if right_stopped else current_right_speed)

            # Si el sensor derecho detecta la línea
            if right_count >= CONSECUTIVE and not right_stopped:
                right_stopped = True
                print("Sensor derecho detectó línea -> detener motor derecho y esperar al izquierdo.")
                tank_drive.on(0 if left_stopped else current_left_speed, 0)

            # Si ambos sensores ya detectaron la línea, salir
            if left_stopped and right_stopped:
                print("Ambos sensores detectaron la línea -> detener completamente.")
                tank_drive.off()
                break

            time.sleep(0.02)

        # 3) Retroceder con la instrucción solicitada: on_for_rotations(15, 15, 0.87)
        print("Paso 3: retroceder con on_for_rotations(15, 15, 0.87)")
        # Según la convención de velocidades del usuario, 15 puede corresponder a retroceso.
        ...
        # El valor de 'degrees' es aproximado y puede necesitar ajuste según la geometría del robot.
        # Aquí usamos 3000 grados de motor como aproximación para 270° de giro del robot.
        tank_drive.on_for_degrees(20, -20, 1050)
        tank_drive.off()
